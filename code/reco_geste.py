...
CAMERA_INDEX = 0 # 0 pour la webcam intégrée à l'ordinateur
...
COMMANDES = {'gauche':'g', 'droite':'d', 'avance':'a', 'avance_rapide':'f',
             'recule':'r', 'stop':'s', 'rien':'s'}
...
def init():
    arduino = serial.Serial('/dev/ttyACM0', baudrate=9600, timeout=2)  # Mettre à jour avec le bon COM
    # Extract Zip file
...
def envoyer_commande_au_robot(class_command, arduino):  # avec class_command = 'left', 'right', ...
    ch_for_bluetooth = COMMANDES[class_command]  # cmd = 'a' ou 'd' ou ...
